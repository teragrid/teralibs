...
for repo in "${REPOS[@]}"; do 
	...
	git remote add -f "$repo" "https://github.com/teragrid/go-${repo}"
...
got replace "teragrid/go-common" "teragrid/go-common/common"
for repo in "${REPOS[@]}"; do 

	if [[ "$repo" != "flowrate" ]]; then
		got replace "teragrid/go-${repo}" "teragrid/go-common/${repo}"
	else
		got replace "teragrid/go-${repo}/flowrate" "teragrid/go-common/flowrate"
	fi
done
